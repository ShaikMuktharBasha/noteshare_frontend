...
const ForgotPasswordPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage(res.message + (res.token ? ` (token: ${res.token})` : ''));
    } catch (err) {
      setError(err.response?.data?.message || 'Unable to request reset');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="mx-auto max-w-5xl">
      ...
      >
        <h2 className="mb-4 text-2xl font-semibold text-white">Forgot password</h2>
